                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(item => {
                            ...
                            div.textContent = "ID: " + item.id + " | Title: " + (item.title || item.name);

                            // رویداد کلیک برای انتخاب آیتم
                            div.addEventListener('click', function () {
                                document.getElementById('related_id').value = item.id; // مقدار را در فیلد مخفی قرار می‌دهد

                                // حذف انتخاب از سایر موارد
                                document.querySelectorAll('.item').forEach(el => el.classList.remove('selected'));
                                this.classList.add('selected'); // اضافه کردن کلاس به آیتم انتخاب‌شده
                            });
